...
if (!class_exists('\\ZJPP\\ECPay_EncryptType', false)) {
...
class ECPay_AllInOne
{
    ...
    function AttachNecessaryFields($arParameters)
    {
        return array_merge(
            [
    ...
    function CheckOut($target = "_self")
    {
        ECPay_Send::CheckOut(
            $target,
            $this->AttachNecessaryFields($this->Send),
            $this->SendExtend,
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
    ...
    {
        return ECPay_Send::CheckOutString(
            $paymentButton,
            $target = "_self",
            $this->AttachNecessaryFields($this->Send),
            $this->SendExtend,
    ...
    {
        return ECPay_CheckOutFeedback::CheckOut(
            $this->AttachNecessaryFields($_POST),
            $this->HashKey,
            $this->HashIV,
            0
        );
    ...
    {
        return ECPay_QueryTradeInfo::CheckOut(
            $this->AttachNecessaryFields($this->Send),
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
    ...
    {
        return ECPay_QueryPeriodCreditCardTradeInfo::CheckOut(
            $this->AttachNecessaryFields($this->Send),
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
    ...
    function DoAction()
    {
        return ECPay_DoAction::CheckOut(
            $this->AttachNecessaryFields($this->Action),
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
        );
    ...
    function AioCapture()
    {
        return ECPay_AioCapture::Capture(
            $this->AttachNecessaryFields($this->Capture),
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
        );
    ...
    {
        ECPay_TradeNoAio::CheckOut(
            $target,
            $this->AttachNecessaryFields($this->TradeNo),
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
        );
    ...
    function QueryTrade()
    {
        return ECPay_QueryTrade::CheckOut(
            $this->AttachNecessaryFields($this->Trade),
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
        );
    ...
    {
        ECPay_FundingReconDetail::CheckOut(
            $target,
            $this->AttachNecessaryFields($this->Funding),
            $this->HashKey,
            $this->HashIV,
            $this->ServiceURL
        );
    ...
    {
        return ECPay_CreateTrade::CheckOut(
            $this->AttachNecessaryFields($this->Send),
            $this->SendExtend,
            $this->HashKey,
            $this->HashIV,
...
abstract class ECPay_Aio
{
    ...
    {
        $response = wp_remote_post($ServiceURL, [
            'method' => 'POST',
    ...
    protected static function OutputHtml($target = "_self", $arParameters = array(), $ServiceURL = '', $szCheckMacValue = '', $paymentButton = '')
    {
        $szHtml =  '<!DOCTYPE html>';
        $szHtml .= '<html>';
        $szHtml .=     '<head>';
        ...
        $szHtml .=     '</head>';
        $szHtml .=     '<body>';
        $szHtml .=         '<form id="__ecpayForm" method="post" target="' . esc_attr($target) . '" action="' . esc_attr($ServiceURL) . '">';

        foreach ($arParameters as $keys => $value) {
            $szHtml .=         '<input type="hidden" name="' . esc_attr($keys) . '" value="' . htmlentities($value) . '" />';
        }

        $szHtml .=             '<input type="hidden" name="CheckMacValue" value="' . esc_attr($szCheckMacValue) . '" />';

        if (!empty($paymentButton)) {
            $szHtml .=          '<input type="submit" id="__paymentButton" value="' . esc_attr($paymentButton) . '" />';
        }
...
class ECPay_Send extends ECPay_Aio
{
    ...
    {
        //宣告付款方式物件
        $PaymentMethod    = '\\ZJPP\\ECPay_' . $arParameters['ChoosePayment'];
    ...
    static function CheckOut($target = "_self", $arParameters = array(), $arExtend = array(), $HashKey = '', $HashIV = '', $ServiceURL = '')
    {
        ...
        echo parent::OutputHtml($target, $arParameters, $ServiceURL, $szCheckMacValue, '');
        exit;
    }
    ...
    static function CheckOutString($paymentButton = 'Submit', $target = "_self", $arParameters = array(), $arExtend = array(), $HashKey = '', $HashIV = '', $ServiceURL = '')
    {
        $arParameters = self::process($arParameters, $arExtend);
        $szCheckMacValue = ECPay_CheckMacValue::generate($arParameters, $HashKey, $HashIV, $arParameters['EncryptType']);
        return parent::OutputHtml($target, $arParameters, $ServiceURL, $szCheckMacValue, $paymentButton);
    }
}
...
if (!class_exists('\\ZJPP\\ECPay_CheckMacValue', false)) {
    ...
    class ECPay_CheckMacValue
    {
        ...
        public static function generate($arParameters = array(), $HashKey = '', $HashIV = '', $encType = 0)
        {
            ...
            if (isset($arParameters)) {
                ...
                uksort($arParameters, array('\\ZJPP\\ECPay_CheckMacValue', 'merchantSort'));
        ...
        /**
         * Replace special char in the input string for base64 encoding
         *
         * @param  string $sParameters
         * @return string
         */
